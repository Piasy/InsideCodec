...
@implementation RTCVideoEncoderH264 {
  RCTestConfig* _testConfig;
  int64_t _rcSign;
  dispatch_source_t _rcTimer;
...
- (instancetype)initTestConfig:(RCTestConfig*)config {
  if (self = [super init]) {
    ...
    xinfo2(
        "Test config: Config{updateBr=%d, initBr=%d, brStep=%d, "
        "outputWidth=%d, outputHeight=%d, outputFps=%d, "
        "outputKeyFrameInterval=%d}",
        config.updateBr, config.initBr, config.brStep, config.outputWidth,
        config.outputHeight, config.outputFps, config.outputKeyFrameInterval);

    //_bitrateAdjuster.reset(new webrtc::BitrateAdjuster(.5, .95));
    _packetizationMode = RTCH264PacketizationModeNonInterleaved;
    _profile = kVTProfileLevel_H264_High_3_1;
...
- (NSInteger)startEncodeWithSettings:(RTCVideoEncoderSettings *)settings
  ...
  _width = _testConfig.outputWidth;
  _height = _testConfig.outputHeight;
  _mode = settings.mode;

  // We can only set average bitrate on the HW encoder.
  _targetBitrateBps = _testConfig.initBr * 1000;  // startBitrate is in kbps.
  //_bitrateAdjuster->SetTargetBitrateBps(_targetBitrateBps);

  // TODO(tkchin): Try setting payload size via
  // kVTCompressionPropertyKey_MaxH264SliceBytes.

  if (_testConfig.updateBr) {
      _rcTimer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0,
                                        dispatch_get_main_queue());
      if (_rcTimer) {
          dispatch_source_set_timer(
              _rcTimer, dispatch_walltime(NULL, 5ull * NSEC_PER_SEC),
              5ull * NSEC_PER_SEC, 0);
          __weak RTCVideoEncoderH264* weakSelf = self;
          dispatch_source_set_event_handler(_rcTimer, ^{
              RTCVideoEncoderH264* strongSelf = weakSelf;
              if (strongSelf) {
                  if (strongSelf->_targetBitrateBps >
                      strongSelf->_testConfig.initBr * 2000) {
                      strongSelf->_rcSign = -1;
                  } else if (strongSelf->_targetBitrateBps <
                             strongSelf->_testConfig.initBr * 1000) {
                      strongSelf->_rcSign = 1;
                  }
                  strongSelf->_targetBitrateBps +=
                      strongSelf->_rcSign * strongSelf->_testConfig.brStep *
                      1000;
                  [strongSelf setBitrateBps:strongSelf->_targetBitrateBps];
                  xinfo2("update bitrate %d",
                         strongSelf->_targetBitrateBps / 1000);
              }
          });
          dispatch_resume(_rcTimer);
      }
  }
...
- (NSInteger)encode:(RTCVideoFrame *)frame
  ...
  encodeParams.reset(new RTCFrameEncodeParams(self,
                                              ...
                                              _width,
                                              _height,
                                              frame.timeStampNs / kNumNanosecsPerMillisec,
                                              frame.timeStamp,
                                              frame.rotation));
...
- (void)configureCompressionSession {
  ...
  SetVTSessionProperty(_compressionSession, kVTCompressionPropertyKey_MaxKeyFrameInterval, _testConfig.outputKeyFrameInterval * _testConfig.outputFps);
  SetVTSessionProperty(
      _compressionSession, kVTCompressionPropertyKey_MaxKeyFrameIntervalDuration, _testConfig.outputKeyFrameInterval);
}
...
- (void)frameWasEncoded:(OSStatus)status
  ...
  xinfo2("reportEncodedImage %d %zu %lld", isKeyframe, block_buffer_size, renderTimeMs * 1000LL);
